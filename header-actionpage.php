...
<body <?php body_class(); ?>>
    ...
    <header class="section header actionpage" role="banner">
		...
		<div class="branding">
                    <?php if ( ! is_home() ) { ?>
                    <a href="<?php echo home_url( '/' ); ?>" title="<?php echo esc_attr( $defaultoptions['default_text_title_home_backlink'] ); ?>" rel="home" class="logo">
                    <?php } ?>
                        <h1><img src="<?php echo esc_url( get_header_image() ); ?>" alt="<?php echo esc_attr( get_bloginfo( 'name' ) ); ?>"></h1>
                    <?php
                      if ( ! is_home() ) { ?> </a>  <?php }
