...
<div class="section content" id="main-content">
  <div class="row">
    <div class="content-primary" role="main">
	  ...
	  $args = wp_parse_args( $args, array( 'posts_per_page' => $numentries ) );
      }

      $home_main_query = new WP_Query( $args );

      $continuelinks = '';
      while ($home_main_query->have_posts() && $i<$numentries) : $home_main_query->the_post();
	  $i++;
          $output = '';
      ...
      // Reset Query
	   ...
	   if (isset($options['artikelstream-title-main']) && (strlen($options['artikelstream-title-main'])>0)) {
        ...
        if ($options['artikelstream-type']>0) {
		 ...
		 $z=1;
		 $linktipp_query = new WP_Query(  array( 'post_type' => array('linktipps'), 'posts_per_page' => $numentries ) );
		 global $post;


		 $linktippout .= '<div class="columns">';
		 while ($linktipp_query->have_posts() && $i<$numentries) : $linktipp_query->the_post();
		     $i++;
		    ...
		    echo "</div>\n";
             ...
             if (($options['artikelstream-type']==2) && ($options['artikelstream-show-second']==1)) {
                 /* Ausnahme-Cats */
                    ...
                    $second_stream_query = new WP_Query( array( 'cat' => $poscatliste, 'posts_per_page' => $numentries ) );
		    $i=0;
		    $cols = array();
		    $col=0;
		    $continuelinks = '';
                  while ($second_stream_query->have_posts() && $i<$numentries) : $second_stream_query->the_post();
                      $i++;
