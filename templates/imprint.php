	...
	<?php } ?>
          ...
          <p>
             <?php _e('This website is brought to you by:','piratenkleider'); ?>
               <?php echo esc_html( $options['impressumdienstanbieter'] ); ?>.
          </p>
           <?php } else { ?>
          ...
          <p><?php _e('Responsible for this website:','piratenkleider'); ?>
                <?php echo esc_html( $options['impressumperson'] ); ?>.
          </p>
           <?php } ?>
            ...
            <address>
                <?php echo esc_html( $options['posttitel'] ) ?><br>
                <?php  if ((isset($options['postperson'])) && (strlen(trim($options['postperson']))>4)) {
                    echo esc_html( $options['postperson'] ). "<br>";
                  }
                  echo esc_html( $options['poststrasse'] )."<br>";
                  echo esc_html( $options['poststadt'] )."<br>";
                ?>
            </address>
           <?php } ?>
           ...
           <?php if ((isset($options['kontaktemail'])) && (strlen(trim($options['kontaktemail']))>4)) { ?>
                <li>E-Mail: <a href="mailto:<?php echo esc_attr( $options['kontaktemail'] ); ?>"><?php echo esc_html( $options['kontaktemail'] ); ?></a></li>
          <?php } else { ?>
                <li>E-Mail: <a href="mailto:<?php echo esc_attr( get_bloginfo( 'admin_email' ) ); ?>"><?php echo esc_html( get_bloginfo( 'admin_email' ) ); ?></a></li>
           <?php }
             if ((isset($options['kontakttelefon'])) && (strlen(trim($options['kontakttelefon']))>4)) { ?>
             <li>Telefon: <?php echo esc_html( $options['kontakttelefon'] ); ?></li>
            <?php }
             if ((isset($options['kontaktfax'])) && (strlen(trim($options['kontaktfax']))>4)) { ?>
             <li>Fax: <?php echo esc_html( $options['kontaktfax'] ); ?></li>
            <?php } ?>
           ...
           <?php if ((isset($options['ladungtitel'])) && (strlen(trim($options['ladungtitel']))>1)
            ...
            <h2><?php _e('Postal address for matters in law','piratenkleider'); ?></h2>
             <address>
                 <?php
                echo esc_html( $options['ladungtitel'] )."<br>";
                if ((isset($options['ladungperson'])) && (strlen(trim($options['ladungperson']))>4)) {
                echo esc_html( $options['ladungperson'] )."<br>";
                }
                echo esc_html( $options['ladungstrasse'] )."<br>";
                echo esc_html( $options['ladungstadt'] )."<br>";
                ?>
            </address>
          ...
          <?php } else {
            ...
            <h2><?php _e('Postal address for matters in law','piratenkleider'); ?></h2>
             <address>
                <?php
                echo esc_html( $options['posttitel'] )."<br>";
...
<ul>
    ...
    ?>
    <li><a class="extern" href="<?php echo esc_url( $theme_data['URI'] ); ?>">Wordpress Theme <?php echo esc_html( $theme_data->Name ); ?></a>, Version <?php echo esc_html( $theme_data->Version ); ?>
